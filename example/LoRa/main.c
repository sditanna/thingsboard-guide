...
void gen_val(char* payload){
    ...
    snprintf(payload, strlen(MSG), MSG, device1, (unsigned long long int)time(NULL), "temperature", gen_ran(-50, 50),"humidity", gen_ran(0,100),
...
static int _cmd_loramac(int argc, char **argv)
{
    ...
    // our case for environmental station
        ...
        while (true) {
            ...
            // the sleep action should get an high value to satisfy the LoRa requirements
            xtimer_sleep(180);
        }
        return 0;
    }
